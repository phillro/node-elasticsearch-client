...
ElasticSearchClient.prototype.getAliases = function(indexName, options, cb) {
    //Pull the callback and set it false to not clobber id.
    if(arguments.length > 0 && typeof arguments[arguments.length-1]=='function'){
        cb=arguments[arguments.length-1];
        arguments[arguments.length-1]=false;
    ...
    if (typeof indexName !== 'string') {
      indexName = null;
      if (typeof indexName !== 'function') options = indexName;
    }

    var path = (indexName) ? '/' + indexName + '/_aliases' : '/_aliases' ;
    var qs = '';
    var opts = options || {}
...
/** The _mapping endpoint is prefixed by an optional index and or type name */
function computeMappingEndpointPath(indexName, typeName, options) {
    if (Array.isArray(typeName)) {
        typeName = typeName.join(',');
    }
    indexName = indexName ? '/' + indexName : '/_all';
...
ElasticSearchClient.prototype.getMapping = function(indexName, typeName, options, cb) {
    ...
    var path = computeMappingEndpointPath(indexName, typeName, options);
    return this.createCall({path:path, method:'get'}, this.clientOptions, cb);
}
...
ElasticSearchClient.prototype.putMapping = function(indexName, typeName, mapping, options, cb) {
    ...
    //note that here indexName and typeName must be specified
    //we let elasticsearch asnwer something went wrong
    var path = computeMappingEndpointPath(indexName, typeName, options);
...
ElasticSearchClient.prototype.getSegments = function(indexName, options, cb) {
    //Pull the callback and set it false to not clobber id.
    if(arguments.length > 0 && typeof arguments[arguments.length-1]=='function'){
        cb=arguments[arguments.length-1];
        arguments[arguments.length-1]=false;
