...
River = require('./calls/river')
ElasticSearchCall = require('./calls/elasticSearchCall.js');

/**
 * options.auth.setHeaders optional function to set the authorization header(s).
 * If it does not exist but options.auth exists, default to Basic Authentication.
 */
function ElasticSearchClient(options) {
    this.clientOptions = options || {}
    if (typeof options.auth === 'object' && options.auth !== null && typeof options.auth.setHeaders !== 'function') {
        options.auth.setHeaders = function(call, request, reqOptions) {
            request.setHeader("Authorization", "Basic " + new Buffer(options.auth.username + ":" + options.auth.password).toString('base64'));
        };
    }
}
